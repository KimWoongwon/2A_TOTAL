			...
			if (Login())
			{
				while (mNowUser->GetLoginState())
				{
					...
					case ITEMBUY:
						mNowUser->DisplayCharacterInfo(false);
						int temp;
						cout << "아이템을 구매할 캐릭터 번호 : ";
						cin >> temp;
						ItemStore::GetInstance()->ShowShop();
						mNowUser->GetChar(temp)->BuyItem(ItemStore::GetInstance()->Sell());
						break;
					case ITEMSELL:
						mNowUser->DisplayCharacterInfo(false);
						int temp1;
						cout << "아이템을 판매할 캐릭터 번호 : ";
						cin >> temp1;
						ItemStore::GetInstance()->Buy(mNowUser->GetChar(temp1)->SellItem());
						break;
					case LOGOUT:
						mNowUser->SetLogout();
						break;
