...
void ItemDataBase::Initialize()
{
	...
	ItemListCount++;
	ItemList->Insert(new Weapon("금으로 수놓은 기사의 플레이트 갑옷", new STATUS(1000, 100, 0, 100), 15000, CODE_WARRIOR));
	ItemListCount++;
	ItemList->Insert(new Armor("여신의 가호를 받은 로브", new STATUS(200, 500, 0, 50), 5000, CODE_WARRIOR));
	ItemListCount++;
}
