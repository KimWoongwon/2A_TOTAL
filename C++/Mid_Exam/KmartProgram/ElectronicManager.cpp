...
#include "Global.h"
#include "Product.h"
ElectronicManager* ElectronicManager::Instance = nullptr;
ElectronicManager::ElectronicManager() {}
ElectronicManager::~ElectronicManager() {}
void ElectronicManager::Initialaze() {}

void ElectronicManager::Create()
{
...
void ElectronicManager::Destroy()
{
...
ElectronicManager* ElectronicManager::GetInstance()
{
...
// 재고를 생성해 입고하는 함수입니다.
void ElectronicManager::InseartItem()
{
	char name[20];
	char date[20];
	int count = 0;
	...
	// 데이터베이스에 ELECTRONIC 리스트에 저장합니다.
	DataBaseManager::GetInstance()->Input(temp, ELECTRONIC);
	cout << "입고중..." << endl;
	Sleep(500);
	system("cls");

}
void ElectronicManager::DeleteItem()
{
	int select = 0;
	// 삭제메뉴 출력
	// 모든재고를 한번에 출력할지
	...
	if (select == 1)
	{
		while (1)
		{
			data = DataBaseManager::GetInstance()->GetData(ELECTRONIC);
			if (data != nullptr)
			{
				...
				{
					DataBaseManager::GetInstance()->Remove(data, ELECTRONIC);
					break;
				}
			}
...
void ElectronicManager::Search()
{
	// 만약 해당 리스트의 카운트가 0이라면 재고가 없기때문에 검색의 의미가 없습니다.
	// 따라서 해당함수를 종료합니다.
	if (DataBaseManager::GetInstance()->isEmpty(ELECTRONIC))
	{
		cout << "가전제품 재고가 비었습니다." << endl;
		return;
	}
	...
	{
		// 데이터를 넘겨받아 검사합니다.
		tempproduct = DataBaseManager::GetInstance()->GetData(ELECTRONIC);
...
void ElectronicManager::Display()
{
	cout << "< 물품 목록 >" << endl;
	cout << "-------------------------------" << endl;
	...
	{
		// 데이터를 넘겨받아서 모두 출력합니다.
		temp = DataBaseManager::GetInstance()->GetData(ELECTRONIC);
...
void ElectronicManager::OverDateSearch(const char* p_str)
{
